...
$dial_timeout_number = AppletInstance::getValue('dial-timeout-number');//added
$dial_callerId = AppletInstance::getValue('dial-callerId'); //added
		...
		case DIAL_STATE_DIAL:
			...
			if ($state[DIAL_NUMBER_INDEX] < count($numbers))
			{
				// There are still more numbers left to try
					...
					$dial = $response->addDial( //added - important!
					                   array(
					                       'action' => current_url(), 
